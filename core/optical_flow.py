...
class OpticalFlowDetector:
    ...
    def __init__(self, flow_cfg: dict) -> None:
        ...
        self.last_masked_flow_x: float | None = None  # Phase 3: mask 픽셀 평균 수평 이동량
    ...
    def update(
        ...
        self.last_roi_box = (x1, y1, x2, y2)

        if self._prev_box is not None:
            self.last_jump_px = self._box_jump_px(self._prev_box, (x1, y1, x2, y2))
        ...
        if self._prev_roi_gray is None or self._prev_roi_gray.shape != roi_gray.shape:
            self._start_new_track(roi_gray, (x1, y1, x2, y2), reason="new_track", clear_history=False)
            return self._check(), self.last_rms
        ...
        self.last_flow = flow
        self.last_masked_flow_x = self._compute_masked_flow_x(flow, x1, y1, mask_xy)

        magnitude = np.sqrt(flow[..., 0] ** 2 + flow[..., 1] ** 2)
        rms = float(np.sqrt(np.mean(magnitude**2)))
        self.last_rms = rms

        # EMA smoothing: dampens single-frame spikes, preserves sustained motion
        ...
        self._prev_box = (x1, y1, x2, y2)
        return self._check(), rms

    def _handle_gap(self, reason: str, *, clear_history: bool) -> None:
        self.last_reset_reason = reason
        self.last_rms = 0.0
        self.last_smoothed_rms = self._ema_rms  # preserve EMA across brief disappearances
        self.last_flow = None
        self.last_masked_flow_x = None
        self.last_roi_box = None
        self._prev_roi_gray = None
        self._prev_box = None
        # do NOT reset _ema_rms: brief gap (steam/hand) should not lose motion state
        if clear_history:
            self._cv_hist.clear()
    ...
    def _start_new_track(
        ...
        *,
        ...
        self.last_flow = None
        self.last_masked_flow_x = None
        # only reset EMA on true target jump; shape changes are minor YOLO bbox variance
        if reason == "box_jump":
            self._ema_rms = 0.0
            self.last_smoothed_rms = 0.0
        self._prev_roi_gray = roi_gray
    ...
    def _compute_masked_flow_x(
        ...
        """딸랑이 세그멘테이션 폴리곤 내부 픽셀만 추출해 평균 수평 이동량 반환.

        mask_xy: 전체 프레임 좌표계의 폴리곤 (N, 2)
        Returns None if mask unavailable or too small.
        """
        if mask_xy is None or len(mask_xy) < 3:
            return None
        rh, rw = flow.shape[:2]
        # 전체 프레임 좌표 → ROI 내부 좌표로 변환
